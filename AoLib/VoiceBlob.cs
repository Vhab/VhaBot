...
namespace AoLib
{
	...
	public class VoiceBlob
	{
		...
		{
			this.tell = FromTell;
			if (blob == null || blob == String.Empty || blob.Trim().Length == 0)
				return;

            buildBlob(Encoding.GetEncoding("iso-8859-1").GetBytes(blob));
		}
		...
		public byte[] GetBytes(bool forTell)
		{
            ...
            AoString str = new AoString(Encoding.GetEncoding("iso-8859-1").GetString(ret));
			return str.GetBytes();
		}
		public bool isTell
		{
