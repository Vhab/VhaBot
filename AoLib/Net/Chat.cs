...
namespace AoLib.Net
{
    ...
    public class Chat
    {
        ...
        protected virtual void OnChannelJoinEvent(ChannelJoinEventArgs e)
        {
            ...
            switch (e.GroupTypeShort)
            {
                ...
                case -32764:
                    ...
                    break;
                case -32700:
                    e.GroupType = ChannelType.Japanese;
                    break;
                case -32412:
        ...
        {
            GroupMessagePacket p = new GroupMessagePacket(channelID, text, voice);
            p.Priority = priority;
            this.SendPacket(p);
        }
        ...
        public virtual void SendFriendAdd(UInt32 userID, bool temp)
        {
            ...
            BuddyStatusPacket p = new BuddyStatusPacket(userID, true, temp);
        ...
        public virtual void SendPrivateChannelInvite(UInt32 userID)
        {
            if (userID == this._id)
                return;
            SimpleIdPacket p = new SimpleIdPacket(Packet.Type.PRIVGRP_INVITE, userID);
        ...
        {
            EmptyPacket p = new EmptyPacket(Packet.Type.PRIVGRP_KICKALL);
            p.Priority = PacketQueue.Priority.Urgent;
            this.SendPacket(p);
        }
        ...
        {
            PrivateGroupMessagePacket p = new PrivateGroupMessagePacket(channelID, text, voice);
            p.Priority = PacketQueue.Priority.Urgent;
            this.SendPacket(p);
        }
        ...
        public virtual void SendPrivateMessage(UInt32 userID, string text, PacketQueue.Priority priority, VoiceBlob voice)
        {
            if (userID == this._id)
                return;
            TellPacket p = new TellPacket(userID, text, voice);
            p.Priority = priority;
            this.SendPacket(p);
        }
